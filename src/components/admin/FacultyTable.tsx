...
export default function FacultyTable({ members }: FacultyTableProps) {
          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            {members.map((member) => (
              <tr key={member.id} className="hover:bg-gray-50">
                <td className="px-4 py-3 font-semibold text-polibatam-navy">{member.name}</td>
            ...
            {members.length === 0 && (
              <tr>
                <td colSpan={5} className="px-6 py-12 text-center text-gray-500">
                  No faculty members found. Create one to get started.
                </td>
              </tr>
            )}
          </tbody>
